...
import type { JwsAlgorithm, JwkType} from "@iota/identity-wasm/web";
...
export function getCompleteJwkFromKeyPair(
  keyPair: Ed25519Keypair,
  alg: JwsAlgorithm,
) {
  ...
  const privateKeyDecoded = decodeIotaPrivateKey(
    ...
    if (DIDcontroller === did) {
      ...
      for (const service of serviceList) {
        if (service.type().includes("LinkedDomains")) {
          ...
          DID_linked_domain =
            "https://" + DID_linked_domain.replace(/^www\./, "") + "/";
          ...
          if (
            response?.data?.linked_dids ||
            Array.isArray(response.data.linked_dids)
          ) {
            const [jwt] = response.data.linked_dids;

            if (!(typeof jwt !== "string" || jwt.split(".").length !== 3)) {
              const fetchedConfigurationResource =
                DomainLinkageConfiguration.fromJSON(response.data);
